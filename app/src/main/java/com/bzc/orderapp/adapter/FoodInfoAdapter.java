...
public class FoodInfoAdapter extends RecyclerView.Adapter<FoodInfoAdapter.ViewHolder>{
    ...
    public FoodInfoAdapter(List<FoodInfoBean> data, Context context, Map<String,Integer> countList){
        this.mDataList = data;
        this.mContext = context;
        this.mCountList = countList;
    }

    public void setData(Map<String,Integer> countList, List<FoodInfoBean> data){
        this.mCountList = countList;
        this.mDataList = data;
    ...
    /** * 减数量 */
    private synchronized void onSubtraction(FoodInfoAdapter.ViewHolder holder) {
    ...
    /** * 添加数量 */
    private synchronized void onAddition(FoodInfoAdapter.ViewHolder holder) {
    //得到当前购物车数量，然后加1
        int position = holder.getPosition();
        FoodInfoBean bean = mDataList.get(position);
        int i = toInt(holder.number.getText().toString());
